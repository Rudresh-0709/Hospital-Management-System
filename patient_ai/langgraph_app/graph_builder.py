...
from .nodes.appointment_booking_node import (
    handle_appointment,
    handle_booking,
    handle_sql_info,
)
...
from .nodes.chat_memory_node import load_chat_history
...
graph.add_node("Load_History", load_chat_history)  # NEW: Load conversation history


# Entry point and initial flow
graph.set_entry_point("User_setter")
graph.add_edge("User_setter", "Load_History")  # First load chat history
graph.add_edge("Load_History", "Chat_logger_User")  # Then log current user message
...
def route_after_intent(state: HMAIState) -> str:
    ...
    print(
        f"DEBUG ROUTING: booking_stage = {state.booking_stage}, intent = {state.intent}"
    )
...
if __name__ == "__main__":
    test_state = HMAIState(
        user_input="Tell me the list of appointments I have and which are completed.",
        patient_id=39,
    )
