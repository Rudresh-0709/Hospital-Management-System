...
from ...tool_nodes.doctor_tool import (
    medical_diagnosis_to_speciality,
    get_available_slots,
    get_specialised_doctors,
)
...
def handle_booking(state: HMAIState) -> HMAIState:
    print(
        f"Entered handle_booking. Stage: {state.booking_stage}, Doctor: {state.selected_doctor}"
    )
    if state.booking_stage == "choose_doctor" and state.available_doctors:
        ...
        for doctor in state.available_doctors:
            if doctor.lower() in user_input_lower or user_input_lower in doctor.lower():
                ...
                formatted_slots = "\n".join(
                    [f"{i+1}. {slot}" for i, slot in enumerate(state.available_slots)]
                )
    ...
    if state.selected_slot is None:
        ...
        if not state.available_slots:
            ...
            if not state.available_slots and state.selected_doctor:
                clean_name = (
                    state.selected_doctor.replace("Dr. ", "")
                    .replace("Doctor ", "")
                    .strip()
                )
        ...
        if state.appointment_date and state.appointment_time:
            slot_str = (
                f"{state.appointment_date} at {state.appointment_time}".strip().lower()
            )
    ...
    if state.selected_slot:
        try:
            ...
            state.follow_up_required = True
            state.next_prompt = (
                "I had trouble processing that time slot. Please try selecting again."
            )
    ...
    if missing and state.patient_id:
        ...
        if patient:  # None -> patient not found
            state.appointee_name = (
                state.appointee_name
                or patient["first_name"] + " " + patient["last_name"]
            )
            state.appointee_email = state.appointee_email or patient["email"]
            state.appointee_contact = (
                state.appointee_contact or patient["contact_number"]
            )
    ...
    if missing:
        state.follow_up_required = True
        state.next_prompt = (
            "I still need your " + ", ".join(missing) + " to complete the booking."
        )
        print(state.next_prompt)
        return state
